...
nam_snp_plot$Chr <- factor(nam_snp_plot$Chr,
                           levels = chr)
pericentromere<-rbind(c('chr1',110000000,160000000),
                      c('chr2',80000000,130000000),
                      c('chr3',70000000,120000000),
                      c('chr4',80000000,130000000),
                      c('chr5',80000000,130000000),
                      c('chr6',40000000,80000000),
                      c('chr7',40000000,80000000),
                      c('chr8',30000000,70000000),
                      c('chr9',40000000,80000000),
                      c('chr10',30000000,70000000))
...
#nam_snp <- data.frame(read.table(snp_file))
png(paste(prefix,"pericentromere.png",sep="."), width=15, height=22, units="in", res=500)
...
png(paste(prefix,"pericentromere_lowerdiv.png",sep="."), width=15, height=22, units="in", res=500)
ggplot(nam_snp_plot_pericentromere, aes(x=Start/1000000, y=Divergence/1000000, color=NAM_line)) +
  geom_point(cex = 0.3,alpha=0.5) + 
  ylab("Divergence Time (million years)") + 
  xlab("Chromosome (Mb)") + 
...
density_plot <- ggplot(nam_snp_plot_pericentromere, aes(x=Divergence/1000000, color=Chr)) + 
  #geom_jitter(position=position_jitter(0.2),cex=0.005) +
  geom_density() + 
...
png(paste(prefix,"pericentromere.density_distribution.0.05-1.png",sep="."), width=8, height=7, units="in", res=500)
...
png(paste(prefix,"pericentromere.density_distribution.0-0.05.png",sep="."), width=8, height=7, units="in", res=500)
density_plot + stat_peaks(
  data = density_plot_build[['data']][[1]], # take a look at this object
  aes(x = x, y = density),
  colour = "red",
